...
# Rename the column
colnames(Silver) <- c("Open", "High", "Low", "Close", "Adjusted", "Volume")
...
```{r log_return}
epsilon <- 1e-8
Dow_Jones$r <- diff.xts(log(Dow_Jones$Adjusted+epsilon))
Tmsc$r <- diff.xts(log(Tmsc$Adjusted+epsilon))
Pound$r <- diff.xts(log(Pound$Adjusted+epsilon))
Silver$r <- diff.xts(log(Silver$Adjusted+epsilon))
BNB$r <- diff.xts(log(BNB$Adjusted+epsilon))
```
...
finish  <- portfolio_df1$obs[portfolio_df1$date == as.Date("2024-05-31")]
